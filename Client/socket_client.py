...
def client_program(ID_Number):
    ...
    host = '192.168.0.21' # pi IP address
...
if __name__ == '__main__':
    ...
    parser.add_argument("Student_ID", help="Student ID Number")

    args = parser.parse_args()
    ID_Number = args.Student_ID

    threading1 = threading.Thread(target=client_program, args=(ID_Number,))
    threading1.daemon = True
    threading1.start()

while True:
    Button(root, text="Quit", command= lambda: quit(0)).pack()
    root.title("Traffic Capture 101: " + student_id)
    root.mainloop()
